# -*- coding: utf-8 -*-
"""
Created on Thu Apr  6 13:53:49 2017
...
def test_xor():
  ...
  # plot the data
    ...
    plt.figure(2)
    plt.subplot(212)
    plt.scatter(X[:,0], X[:,1], c=prediction,cmap='prism', s=50, alpha=0.5 )
...
def test_donut():
  ...
  # plot the data
    plt.figure(1)
    plt.subplot(211)
    plt.scatter(X[:,0], X[:,1], c=Y,cmap='rainbow', s=20, alpha=0.5)
    plt.title('True data')
    
    plt.figure(2)
    plt.subplot(212)
    plt.scatter(X[:,0], X[:,1], c=prediction,cmap='prism', s=20, alpha=0.5 )
    plt.title('Prediction')
    plt.show()
